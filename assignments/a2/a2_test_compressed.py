...
def test_add()-> None:
    ...
    x.add('ab', 0.5, ['a', 'b']) #TODO minor space error in str
    assert str(x) == \
           "[] (0.5)\n  ['a', 'b'] (0.5)\n    ab (0.5)\n"

    y = CompressedPrefixTree()
...
def test_add_on()-> None:
    ...
    assert x.weight == 0.7
    assert x.value == []
    assert x.subtrees[0].value == ['a','b']
    assert x.subtrees[0].weight == 0.7
    assert x.subtrees[0].subtrees[0].value == 'ab'
    assert x.subtrees[0].subtrees[0].weight == 0.5
    assert x.subtrees[0].subtrees[1].value == ['a', 'b', 'c']
    assert x.subtrees[0].subtrees[1].weight == 0.2
    assert x.subtrees[0].subtrees[1].subtrees[0].value == 'abc'
    assert x.subtrees[0].subtrees[1].subtrees[0].weight == 0.2
...
def test_insert():
    ...
    s = CompressedPrefixTree('average')
    s.insert('a', 0.1, [])
    assert str(s) == '[] (0.1)\n  a (0.1)\n'
    assert len(s) == 1
