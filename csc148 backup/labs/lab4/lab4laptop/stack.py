...
def remove_big(s: Stack) -> None:
    ...
    side_stack = Stack()
    holder = 0
    # Pop everything off <s> and onto <side_stack>, removing greater than 5 as we go.
    while not s.is_empty():
        holder = s.pop()
        if holder <=5:
            side_stack.push(holder)
    # Now pop everything off <side_stack> and back onto <s>.
    while not side_stack.is_empty():
        s.push(side_stack.pop())
    # <s> is now without items greater than 5.
...
def double_stack(s: Stack) -> Stack:
    ...
    """
    new_stack = Stack()
    old_stack_copy = Stack()
    holder = 0
    # Pop everything off <s> and onto <side_stack>, adding a duplicate item after each item.
    while not s.is_empty():
        holder = s.pop()
        new_stack.push(holder)
        new_stack.push(holder)
        old_stack_copy.push(holder)
    while not old_stack_copy.is_empty():
        s.push(old_stack_copy.pop())
    return new_stack
