...
# @given(integers(min_value=2, max_value=20), integers(min_value=0, max_value=20))
# def test_random_arrival_generator_zero(n1: int, n2: int) -> None:
...
# def test_random_arrival_generator_none(n1: int) -> None:
#     """Test the random arrival generator when num_people = 0
#     Test the random arrival generator when num_people = None
#
#     Note that this test just checks that the range of possible values
#     for the random people are correct.
#     """
...
#     num_per_round = None
#     random_generator = RandomArrivals(max_floor, num_per_round)
#     # first hypothesis test of the day:
#     # failed tests n1=0, n2=1
#     # failed tests n1=0, n2=-1
#
#
...
def test_random_arrival_generator_none() -> None:
    """Test the random arrival generator when num_people = 0
    Test the random arrival generator when num_people = None
    ...
    for round_num in [0, 5]:
        ...
        # Check that the right number of people were generated.
        # if num_per_round is None:
        #     assert 0 == len(all_people)
        num_per_round = 0
        assert num_per_round == len(all_people)
...
# @given(integers(min_value=4, max_value=10))
# def test_file_arrival_generator(n1) -> None:
#     """Test the CSV arrival generator for the given sample_arrivals file.
#     """
#     max_floor = n1
#     file_generator = FileArrivals(max_floor, 'sample_arrivals7.csv')
#
...
#             assert people[-1].start == 1
#             assert people[-1].target == 2
#         elif floor == 2:
#             assert len(people) == 1
#             assert people[0].start == 2
#             assert people[0].target == 3
#         else:
#             assert len(people) == 0
#
#     # Next try round 5. One person arrives.
#     round_two = file_generator.generate(2)
#     for floor, people in round_two.items():
#         if floor == 3:
#             assert len(people) == 2
#             assert people[0].start == 3
#             assert people[0].target == 2
#         else:
#             assert len(people) == 0

# @given(integers(min_value=3, max_value=10),integers(min_value=1, max_value=10), integers(min_value=1, max_value=6), integers(min_value=1, max_value=6))
# def test_random_moving_algorithm(n_floors: int, n_elev: int, n_cap: int, n_peep: int) -> None:
...
#         'elevator_capacity': n_cap,
#         # This is likely not used.
#         'num_people_per_round': n_peep,
#         'arrival_generator': FileArrivals(n_floors, 'sample_arrivals7.csv'),
...
#     assert results['total_people'] == 20
#
...
# @given(integers(min_value=5, max_value=10),integers(min_value=1, max_value=10), integers(min_value=1, max_value=6), integers(min_value=1, max_value=6))
# def test_pushy_passenger_moving_algorithm(n_floors: int, n_elev: int, n_cap: int, n_peep: int) -> None:
...
#         # This is likely not used.
#         'num_people_per_round': n_peep,
#         'arrival_generator': FileArrivals(n_floors, 'sample_arrivals8.csv'),
...
if __name__ == '__main__':
    import pytest
    pytest.main(['hyp_a1_sample_test.py'])
