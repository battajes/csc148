...
class BinarySearchTree:
    ...
    def count(self, item: Any) -> int:
        ...
        if self.is_empty():
            ...
            if self._root == item:
                c += 1
            if self._root <= item:
                c += self._right.count(item)
            if self._root >= item:
                c += self._left.count(item)
    ...
    def smaller(self, item: Any) -> List:
        ...
        if self.is_empty():
            ...
            if self._right is not None:
                items.extend(self._right.smaller(item))


            #  and self._right._root < item
            # items.extend([self._root])
            # items.extend(self._left.smaller(item))
        ...
        return items
            ...
            # if self._root == item - 1:
            #
            # if self._root < item:
            #     self._left.smaller(item)
