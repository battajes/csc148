...
class LinkedListL:
    ...
    def __getitem__(self, index: Union[int, slice]) -> Union[Any, LinkedListL]:
        ...
        if isinstance(index, slice):
            if index.start < 0:
                if index.stop > 0 or (
                        index.stop < 0 and index.start >= index.stop):
                    return LinkedListL([])
                elif index.stop < 0 and index.start < index.stop:
                    new_start = self._length + index.start
                    new_stop = self._length + index.stop
                    if new_start < 0:
                        new_start = 0
                    if new_stop < 0:
                        new_stop = 0
                    new_index = slice(new_start, new_stop, index.step)
            else:
                new_index = slice(index.start, index.stop, index.step)
