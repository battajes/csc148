...
class Player:
    ...
    def move(self, cureent, min, max, goal):
        """Move a player.
        """
        raise NotImplementedError

class UserPlayer (Player):
    """A player that moves based on user input.
    ...
    - input value is between minimum and maximum

    """
    def __init__(self, ):

    def move(self, current, min, max, goal) -> int:
        x = input(f("Enter you desired move. Must be inbetween {min} and {max}"))
        return x

class RandomPlayer (Player):
    """ A player that moves a random amount between the minimum and maximum values.
...
class NumberGame:
    ...
    - (self.max_step - self.min_step) >= self.min_step
    """
    goal: int
    min_step: int
    max_step: int
...
def main() -> None:
    ...
    p2 = make_player('p2')
    while True:
        # example of how to violate a representation invariant
        # g = NumberGame(21,-1, 22, (p1,p2))
        # minimum should be a positive value, maximum should be less than the goal
